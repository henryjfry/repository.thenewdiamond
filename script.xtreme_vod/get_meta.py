...
fanart_api_key = Utils.fanart_api_key
	...
	for rss_feed in rss_feed_list:
		...
		for i in NewsFeed.entries:
			...
			if db_result:
				...
				if torr_info:
					Utils.log('RSS_ADDED_MAGNET',torr_info['filename'])
					Utils.write_db(connection=Utils.db_con,url=url, cache_days=cache_days, folder=folder,cache_val=torr_info)
