...
UNRESTRICT_FILE = None
UNRESTRICT_FILE_ID = None
UNRESTRICT_FILE_SIZE = None
original_tot_bytes = None
original_start_time = None
remaining_tot_bytes = None
num_lines = None
...
def curr_percent(remaining_tot_bytes, original_tot_bytes, original_start_time):
	if original_tot_bytes == 0:
		return
	curr_percent = round((remaining_tot_bytes/original_tot_bytes) * 100,2)
	...
	if percent_done == 0:
		return
	time_running = time.time() - original_start_time
	...
	if xbmc_gui:
		...
		xbmcgui.Window(10000).setProperty('num_lines_remaining', str(num_lines))
	return curr_percent

def download_progressbar(url, file_path, remaining_tot_bytes2, UNRESTRICT_FILE_SIZE):
	#from urllib.request import urlretrieve
	global remaining_tot_bytes
	global original_start_time
	print(original_start_time)
	remaining_tot_bytes = remaining_tot_bytes2
	stop_downloader = get_setting('magnet_list').replace('magnet_list.txt','stop_downloader')
	final_remaining_tot_bytes = remaining_tot_bytes - UNRESTRICT_FILE_SIZE

	if os.path.exists(file_path):
		remaining_tot_bytes = remaining_tot_bytes - os.path.getsize(file_path)
	...
	def dlProgress(count, blockSize, totalSize):
		...
		global remaining_tot_bytes
		remaining_tot_bytes = remaining_tot_bytes - blockSize
		curr_percent(remaining_tot_bytes, original_tot_bytes, original_start_time)
	...
	if final_remaining_tot_bytes < remaining_tot_bytes:
		remaining_tot_bytes = final_remaining_tot_bytes
	UNRESTRICT_FILE_SIZE = 0
	sys.stdout.write('\n')
	sys.stdout.flush()
	return file_path, remaining_tot_bytes
