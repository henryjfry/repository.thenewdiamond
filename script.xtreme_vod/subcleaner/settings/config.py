...
#import libs
try: from libs.subcleaner import languages
except: from subcleaner import languages
...
folder = getframeinfo(currentframe()).filename.replace(str(os.path.split(str(getframeinfo(currentframe()).filename))[1]),'')
try: home_dir = Path(libs.__file__).parent.parent
except: home_dir = Path(folder)
try:
