...
class PlayerMonitor(xbmc.Player):
	...
	def onPlayBackStarted(self):
		...
		if decoded_sorttitle_b64 != {}:
			...
			started = decoded_sorttitle_b64.get('diamond_info_started')
			if started is not None:
				self.player_meta['diamond_info_started'] = started
				if not started:
					self.clearProperty('diamond_info_time')
			# script.xtreme_vod_player
			player_flag = decoded_sorttitle_b64.get('diamond_player')
			if player_flag is not None:
				self.player_meta['diamond_player'] = player_flag
				if player_flag:
					self.clearProperty('Next_EP.ResolvedUrl')
