...
class GoogleProvider(BaseProvider):
    ...
    def fetch_data(self, search_str):
        lang = SETTING("autocomplete_lang")
        if not lang:
            lang = 'en'
        url = "search?hl=%s&q=%s&json=t&client=serp" % (lang, quote_plus(search_str))
        if self.youtube:
            url += "&ds=yt"
...
class LocalDictProvider(BaseProvider):
    ...
    def get_predictions(self, search_str):
        ...
        path = os.path.join(ADDON_PATH, "data", "common_%s.txt" % (local if local else "en"))
