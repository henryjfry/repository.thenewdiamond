...
from lib.provider import tvdb # import on behalf of searching when there's no ID
from lib.provider.local import local
from lib.report import create_report
from lib.script_exceptions import *
from lib.settings import get_limit, get_settings, check
...
setting = lib.settings.get_settings()
...
class Main:
    ...
    def download_artwork(self, media_list, providers):
        ...
        for currentmedia in media_list:
            ...
            if (not currentmedia['mediatype'] == 'tvshow' and
                ...
                currentmedia['name'] = xbmc.getInfoLabel("ListItem.OriginalTitle")
                log('Processing media with the original title:  %s' % currentmedia['name'])                
                currentmedia['id'] = tmdb._search_movie(currentmedia['name'],currentmedia['year'])
            ...
            # If correct ID found and don't already have all artwork retrieve from providers
                ...
                for self.provider in providers:
                    log('=== Checking: %s ===' % self.provider.name)				
                    ...
                    while not artwork_result == 'pass' and not artwork_result == 'skipping':
                        ...
                        try:
                            if currentmedia['mediatype'] == 'tvshow':   ####### @burekas
                                _originaltitle = self.getTVshowOriginalTitleByJSONandDBid()
                                log('TV Show Original Title after checking Kodi Library database: %s' % _originaltitle)								
                                if _originaltitle == '':
                                    _originaltitle = self.getTVshowOriginalTitleByTMDBapi()
                                    log('TV Show Original Title after checking TMDB: %s' % _originaltitle)
                                if _originaltitle != '':
                                    log('Start seraching for TV Show by original title')								
                                    if (self.provider.name == 'TVDB' or self.provider.name == 'FANART.TV'):   ####### @burekas
                                        currentmedia['id'] = tvdb._search_tv(_originaltitle, currentmedia['year'])
                                        log('TVDB ID is: %s' % currentmedia['id'])
                                    elif self.provider.name == 'TMDB':
                                        currentmedia['id'] = tmdb._search_tv(_originaltitle, currentmedia['year'])
                                        log('TMDB ID is: %s' % currentmedia['id'])
                                log('Fallback - Trying to get image for TV Show with Kodi Library database ID: %s' % currentmedia['id']) 

                            temp_image_list = self.provider.get_image_list(currentmedia['mediatype'], currentmedia['id']) ####### @burekas
                            #pass
                    ...
                    if art_item['art_type'] in artwork['art_type']:
                        ...
                        if startup['mode'] in ['gui', 'customgui'] and not art_item['art_type'] in ['extrafanart', 'extrathumbs']:
                            ...
                            if (setting['files_local'] and not
                                ...
                                limited = list(filter(art_item['art_type'],
                                                 startup['mediatype'],
    ...
    def getTVshowOriginalTitleByJSONandDBid(self):   ####### @burekas
        #Only TMDB scraper gives the original title	
        ...
        try:
            ...
            tvshowDBID = xbmc.getInfoLabel("ListItem.DBID")
    ...
    def getTVshowOriginalTitleByTMDBapi(self):   ####### @burekas
        try:	
            originalTitle = ''
            labelTVShowTitle = xbmc.getInfoLabel("ListItem.TVShowTitle")
            labelYear = xbmc.getInfoLabel("ListItem.Year")
            tmdbKey = '653bb8af90162bd98fc7ee32bcbbfb3d'
            filename = 'wizdom.search.tmdb.%s.%s.%s.json' % ("tv",self.lowercase_with_underscores(labelTVShowTitle), labelYear)
            if labelYear > 0:
            ...
            json = self.caching_json(filename,url)
            #log(JSON result: %s' % json)
            ...
            #log(originalTitle : %s' % originalTitle)
    ...
    def lowercase_with_underscores(self,_str):   ####### @burekas
    ...
    def caching_json(self, filename, url):   ####### @burekas
        json_file = path.join(__temp__, filename)
        #log('BBB: jsonfile : %s' % json_file)
