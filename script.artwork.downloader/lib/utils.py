...
def dialog_msg(action,
    ...
    if not background:
        ...
        if action == 'update':
            dialog.update(percentage, line1 +'\n'+ line2 +'\n'+ line3)
        if action == 'close':
            dialog.close()
        if action == 'iscanceled':
            if dialog.iscanceled():
                ...
                return False
        if action == 'okdialog':
            xbmcgui.Dialog().ok(line0, line1 +'\n'+ line2 +'\n'+ line3)
        if action == 'yesno':
            return xbmcgui.Dialog().yesno(line0, line1 +'\n'+ line2 +'\n'+ line3, nolabel, yeslabel)
    if background:
        if (action == 'create' or action == 'okdialog'):
            if line2 == '':
                ...
                msg = line1 + ': ' + line2
            if cancelled == False:
                xbmc.executebuiltin("Notification(%s, %s, 7500, %s)" % (line0, msg, __icon__))
