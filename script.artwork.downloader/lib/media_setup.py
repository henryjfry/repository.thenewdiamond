...
def _media_unique(media_type, dbid):
    ...
    if media_type == 'tvshow':
        json_query = xbmc.executeJSONRPC('{"jsonrpc": "2.0", "method": "VideoLibrary.GetTVShowDetails", "params": {"properties": ["file", "imdbnumber", "art", "year"], "tvshowid":%s}, "id": 1}' %dbid)    ####### @burekas
        ...
        if 'result' in jsonobject and 'tvshowdetails' in jsonobject['result']:
            ...
            json_query_season = str(json_query_season)
        ...
        if 'result' in jsonobject and 'moviedetails' in jsonobject['result']:
            ...
            disctype = media_disctype(item.get('file','').lower(),
