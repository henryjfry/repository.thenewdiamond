...
def next_ep_play(show_title, show_season, show_episode, tmdb):
	...
	def meta_process(meta):
		...
		try: runtime_seconds = int(info['runtime']) * 60
		...
		return show_title, episode_thumb, air_date, resume_progress_seconds, runtime, rating, genre, plot, episode_id, year, episode_name, tv_maze_rating, alternate_titles, imdb_id, tvdb_id, tmdb_id, show_title_clean, show_title, info, tmdb_response, extended_tvshow_info_response
	...
	show_title, episode_thumb, air_date, resume_progress_seconds, runtime, rating, genre, plot, episode_id, year, episode_name, tv_maze_rating, alternate_titles, imdb_id, tvdb_id, tmdb_id, show_title_clean, show_title, info, tmdb_response, extended_tvshow_info_response = meta_process(new_meta)
...
def next_ep_play_movie(movie_year, movie_title, tmdb):
	...
	#torrent_threads = ThreadPool()
	#tools.SUB_FILE = None
	#getSources.Sources(meta).torrent_threads.put(getSources.get_subtitles, arg=(meta, PTN_download))
	#getSources.Sources(meta).torrent_threads._worker_maintenance()
	#getSources.Sources(meta).torrent_threads.wait_completion()
	#getSources.Sources(meta).torrent_threads.terminate()
	#import threading
	#x = threading.Thread(target=getSources.get_subtitles,args=(meta , PTN_download))
	#x.start()
	#while x.is_alive():
	#	meta_test = tools.SUB_FILE
	#new_meta = x.join()

	#del torrent_threads
	#del ThreadPool
	#subfile = getSources.get_subtitles2(meta, PTN_download)
	#subfile2 = subfile.get_subtitle()
	#tools.log(tools.SUB_FILE, new_meta)
	#exit()
